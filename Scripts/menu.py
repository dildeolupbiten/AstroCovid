...
class Menu:
    def __init__(self, master, treeview, frame):
        self.master = master
    ...
    def get_values(items: str = "", data=None, compare: bool = False):
        ...
        if not compare:
            ...
            return [int(i) for i in values], coordinates
        else:
            for item in items:
                for i, j in enumerate(data[1:]):
                    if item == j[0]:
        ...
        if self.treeview.selection():
            ...
            if select == "proportion":
                ...
                if daily_or_total == "Daily":
                    val1 = self.daily_or_total(arr=val1)
                    val2 = self.daily_or_total(arr=val2)
                plot_data(
                    master=self.frame,
                ...
                if daily_or_total == "Daily":
                    for i in range(len(value1)):
                        value1[0][i] = self.daily_or_total(arr=value1[0][i])
                        value2[0][i] = self.daily_or_total(arr=value2[0][i])
                if 2 <= len(value1) < 6:
                    plot_data(
                        master=self.frame,
                        x=self.treeview.times,
                ...
                value, coord = values
                if daily_or_total == "Daily":
                    value = self.daily_or_total(arr=value)
                plot_data(
                    master=self.frame,
                    x=self.treeview.times,
                    y=value,
                ...
                if daily_or_total == "Daily":
                    for i in range(len(value)):
                        value[i] = self.daily_or_total(arr=value[i])
                if 2 <= len(value) < 6:
                    plot_data(
                        master=self.frame,
                        x=self.treeview.times,
